        ...
        import('./costumeManager.js').then(({ costumeManager }) => {
          import('./stateManager.js').then(({ state }) => {
            import('./utils/DataUtils.js').then(({ DataUtils }) => {
              // 初始化临时状态
              costumeManager.originalCostumes = DataUtils.deepClone(state.get("currentCostumes"));
              costumeManager.originalAvailableCostumes = DataUtils.deepClone(costumeManager.availableCostumes);
              costumeManager.tempCostumeChanges = DataUtils.deepClone(state.get("currentCostumes"));
              costumeManager.tempAvailableCostumes = DataUtils.deepClone(costumeManager.availableCostumes);
      ...
      case 7: // 位置配置
        import('./positionManager.js').then(({ positionManager }) => {
          import('./utils/DataUtils.js').then(({ DataUtils }) => {
            ...
            positionManager.tempManualPositions = DataUtils.deepClone(positionManager.manualPositions);
        ...
        break;
      case 8: // 动作/表情配置
        import('./motionExpressionEditor.js').then(({ motionExpressionEditor }) => {
          import('./genericConfigManager.js').then(({ motionManager, expressionManager }) => {
